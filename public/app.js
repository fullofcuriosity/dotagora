...
const EDIT_MODE = path === '/builder' || path.startsWith('/builder/');
...
// ---- Unified State (Server + Local fallback) ----
let STATE = null;

// Lädt zuerst Server-State (wenn vorhanden), sonst localStorage
...
// Getter mit Defaults
function getStateSection(key, defaults) {
  if (!STATE) STATE = {};
  const v = STATE[key];
  if (v == null) STATE[key] = structuredClone ? structuredClone(defaults) : JSON.parse(JSON.stringify(defaults));
  return STATE[key];
}
...
let mods = null; // wird nach bootState gesetzt
...
// conf kommt aus unified STATE
let conf = null; // wird nach bootState gesetzt
...
if (conf.logo) {
  ...
  const circle = document.querySelector('.avatar-circle');
    ...
    // Background video
    if(conf.bgVideo && bgv){
      const src = bgv.querySelector('source');
      if(src){
...
function saveConf(){
  ...
  // ✅ Quelle der Wahrheit: STATE
  STATE.conf = conf;
  persistState();

  // (Optional) für ältere Listener – kann bleiben:
  try { localStorage.setItem(KEY_CONF, JSON.stringify(conf)); } catch {}
...
window.bc?.addEventListener('message', (ev) => {
  const { type } = ev.data || {};
if (type === 'theme') {
  // Prefer STATE, fallback localStorage
  const next = (STATE?.conf) || safeJson(localStorage.getItem(KEY_CONF), {});
  Object.assign(conf, next);
  applyConf();
}
...
edSlogan2?.addEventListener('blur',  () => normalizeOnBlur('edSlogan2','slogan2',200));
  ...
  function setLogo(url){
    const validUrl = url?.trim() || 'avatar.jpg';
...
function loadProducts(){
  return (STATE?.products && Array.isArray(STATE.products)) ? STATE.products : [];
}
function saveProducts(list){
  STATE.products = Array.isArray(list) ? list : [];
  persistState();
  ping('products', {}); // damit Live-Preview reagiert
}
...
// Optional: keine KEY_PRODUCTS Speicherung mehr – STATE ist die Quelle
...
<div class="row">
  ...
  // jetzt ist STATE sicher vorhanden
  products = loadProducts();
  if (!products.length) products = seedFromHtmlIfEmpty();
...
// INIT (STATE-FIRST)
// =====================
async function initStateAndUI(){
  await bootState();

  // Sections mit Defaults erstellen
  conf = getStateSection('conf', confDefaults);
  mods = getStateSection('mods', { booking:false, shop:false, reviews:false, services:false });

  // Shipping sauber klonen + mergen
  shipConf = structuredClone
    ? structuredClone(shipDefaults)
    : JSON.parse(JSON.stringify(shipDefaults));

  const s = getStateSection('shipping', shipDefaults);
  if (s?.zones) shipConf.zones = Object.assign({}, shipConf.zones, s.zones);
  if (s?.free)  shipConf.free  = Object.assign({}, shipConf.free,  s.free);
  ...
  // Builder-Editoren erst JETZT hydraten
  document.dispatchEvent(new Event('state-ready'));
}
...
openBtn?.addEventListener('click', (e) => {
  ...
  waitForQrReady(qrBox, 3000).then((canvas) => {
    drawCenterImageOnCanvas(canvas, 'puzzle.png', { sizeRatio: 0.26 });
  });
...
function drawCenterImageOnCanvas(canvas, src, { sizeRatio = 0.26 } = {}) {
  const ctx = canvas.getContext('2d');
  const S = canvas.width;
  const size = Math.round(S * sizeRatio);
  const x = Math.round((S - size) / 2);
  const y = Math.round((S - size) / 2);
...
// erst nach Server-State laden
document.addEventListener('state-ready', hydrateShipEditor);
